...
error=$(grep -n "ERROR" "$log_file")

echo " $error" 
#For Task 4, the important point is: you need to find repeated ERROR messages, count them, and show the top 5.

error=$(grep "ERROR" "$log_file" | sed 's/.*ERROR //' | sort | uniq -c | sort -nr | head -5)
echo "$error"
